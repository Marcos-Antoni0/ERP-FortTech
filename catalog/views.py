...
class UploadProductsFromXml(View):
    template_name = 'catalog/upload_products_xml.html'
    ...
    def get(self, request):
        ...
        categories = Category.objects.filter(company=user_company, status=1).order_by('name')
    ...
    def post(self, request):
        ...
        if not user_company:
            return JsonResponse(
                {'status': 'failed', 'msg': 'Usuário não está associado a nenhuma empresa.'}
            )

        # Check if the request is for saving data (from the preview)
        if request.content_type == 'application/json':
            try:
                data = json.loads(request.body)
                return self._apply_items(request, user_company, data.get('products', []))
            except json.JSONDecodeError:
                return JsonResponse({'status': 'failed', 'msg': 'JSON inválido.'})

        # Otherwise, it's a file upload for preview
        upload_file = request.FILES.get('file')
        if not upload_file:
            return JsonResponse(
        ...
        default_category_id = request.POST.get('category_id')
        parsed_items, errors = self._parse_xml_items(upload_file, user_company)
        
        if errors:
            # This part is tricky for AJAX, let's return a special template with errors
            # For simplicity now, we assume the js will handle it.
            pass

        categories = Category.objects.filter(company=user_company, status=1).order_by('name')

        # Return only the preview part of the template
        return render(request, 'catalog/upload_products_xml.html', {
            'products_to_import': parsed_items,
            'categories': categories,
    ...
    def _parse_xml_items(self, upload_file, company):
        ...
        for node in root.iter():
            ...
            prod = next((child for child in node if strip_tag(child.tag) == 'prod'), None)
            if prod is None:
                continue
            ...
            price_value = _parse_decimal_cell(find_child_text(prod, 'vUnCom') or find_child_text(prod, 'vProd'))

            price_number = float(price_value) if price_value is not None else 0.0
            price_display = f'{price_number:.2f}'
            items.append({
                'code': code,
                'name': name,
                'price': price_number,
                ...
                'cost_display': price_display,
                'description': find_child_text(prod, 'xProd'),
            })

        if not items:
            errors.append('Nenhum item de produto encontrado no XML informado.')

        return items, errors

    def _apply_items(self, request, company, products_data):
        created = 0
        updated = 0
        errors = []
        ...
        for item in products_data:
            ...
            if not code:
                errors.append(f"Um item está sem código.")
                continue
            
            if not category_id:
                errors.append(f"Selecione uma categoria para o produto {code}.")
                continue

            category = Category.objects.filter(id=category_id, company=company).first()
            if not category:
                errors.append(f"Categoria inválida para o produto {code}.")
                continue
        ...
        if errors:
            return JsonResponse({'status': 'failed', 'msg': ' '.join(errors)})
        
        messages.success(request, f'{created} produtos criados e {updated} atualizados com sucesso.')
        return JsonResponse({'status': 'success'})

upload_products_from_xml = UploadProductsFromXml.as_view()
